...
class Main {
 ...
 public static void main(String[] args)throws IOException{
  ...
  void init() throws IOException{   
    ...
    Database db = new Database("jdbc:sqlite:Cinema.DB");
    
    // Create a route handler to respond to the request (default route)
    server.createContext("/", new RouteHandler("Default route...") );

    // create a route called 'actors' that gets all customer records.
    String sql = "";
    sql  = " Select * from Actors ";
    server.createContext("/actor", new RouteHandler(db,sql) );

    // Create a route called 'directors' that gets all employee records.
    sql  = " Select * from Directors ";
    server.createContext("/director", new RouteHandler(db,sql) ) ;

    // Create a route called 'movies' that gets all employee records.
